...
<template>
  <nav class="fixed top-0 w-full z-50 transition-all duration-500 backdrop-blur-md" :class="[
    navClasses,
      ...
      <div class="flex items-center justify-between h-16">
        <RouterLink :to="{ name: 'Home' }" class="flex items-center space-x-3 group">
          <div class="relative w-8 h-8 animate-logo-float ">
            <img :src="logo || '/path-to-your-logo.svg'" alt="SyntiIQ Logo"
              class="w-full h-full object-contain transition-transform duration-300 group-hover:scale-110 dark:text-white" />

            <div
              class="absolute inset-0 bg-gradient-to-r from-primary/20 to-blue-600/20 blur-xl transition-opacity duration-300 group-hover:opacity-100 opacity-0">
            </div>
          </div>
          <span :class="[
        ...
        <div class="flex items-center space-x-6">
          <button @click="toggleNotifications" class="relative p-2 rounded-full transition-transform hover:scale-105"
            :class="isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'">
            <FontAwesomeIcon icon="bell" :class="[
          ...
          <RouterLink :to="{ name: 'Cart' }" class="relative p-2 rounded-full transition-transform hover:scale-105"
            :class="isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-100'">
            <FontAwesomeIcon icon="shopping-bag" :class="[
              'text-xl transition-colors',
              isDark ? 'text-gray-300' : 'text-gray-700',
          ...
          <div class="relative">
            <button @click="toggleDropdown" class="flex items-center space-x-2 focus:outline-none">
              <div class="relative">
                <img :src="userAvatar || '/path-to-default-avatar.jpg'" alt="User Avatar"
            ...
            <Transition appear :show="showDropdown">
              <div v-if="showDropdown"
                ...
                <div class="py-1">
                    ...
                    <div class="flex items-center justify-between">
                      ...
                      <button @click="changeTheme"
                        ...
                        <span
                          ...
                          <FontAwesomeIcon :icon="isDark ? 'moon' : 'sun'" class="text-xs"
                  ...
                  <button v-for="(item, index) in menuItems" :key="index" @click="item.action"
                    class="flex items-center w-full px-4 py-2 text-sm transition-colors duration-200" :class="[
                      isDark
